...
//Populate Select Key
add_filter('acf/load_field/key=field_5db1803fbc65f', 'acf_load_featured_data_choices');

function acf_load_featured_data_choices( $field ){
     
	// where our new values will live
	$chart_type = get_field_object('field_5db17fbabc65e');
	$field['choices'] = array();
	
	global $wpdb;
	$published = '0';

    $hurumap = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->base_prefix}hurumap_charts where published=%s order by title desc", $published));     
	// dynamic data we want to popuplate with
	foreach($hurumap as $k => $v ) {
		$field['choices'][$v->id] = $v->title;
	}
        // $value = $label
...
function my_acf_input_admin_footer() {
	...
	(function($) {
		...
		$('[id$="field_5db17fbabc65e"]').each(function(){
			$el.on('change', $ (this), function(){
				console.log("I have changed");
				//acf.doAction('acf_load_featured_data_choices');
			});

		});

		// $(chart_type_select).change(function () {
		// 	console.log($chart_type_select);
		// });
